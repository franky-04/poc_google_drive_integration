...
Supporta sia client di tipo 'installed' (desktop) che 'web'.
"""
...
def authenticate(auth_config):
    """
    Gestisce l'autenticazione con Google Drive API, supportando client web e desktop.
    ...
    client_type = auth_config.get('client_type', 'web')  # Default a 'web'
    ...
    if not creds or not creds.valid:
        if creds and creds.expired and creds.refresh_token:
            ...
            # Crea il flow per il tipo di applicazione appropriato
            if client_type == 'web':
                ...
                logger.info("Utilizzo di credenziali per applicazione installata")
            ...
            if headless:
                ...
                if client_type == 'web':
                    ...
                    # Anche se l'URL inizia con http://, verrà gestito correttamente
                    # grazie alla variabile di ambiente OAUTHLIB_INSECURE_TRANSPORT=1
                    flow.fetch_token(authorization_response=redirect_url)
                else:
                    # Per applicazioni desktop in modalità headless
                    auth_url, _ = flow.authorization_url(
                        access_type='offline',
                        prompt='consent'
                    )
                    ...
                    print("\n" + "="*60)
                    print("Autenticazione Google Drive (App Desktop)")
                    print("="*60)
                    print("\nVisita il seguente URL nel tuo browser per autorizzare l'applicazione:")
                    print(f"\n{auth_url}\n")
                    print("Dopo l'autorizzazione, riceverai un codice da incollare qui sotto.")
                    print("="*60 + "\n")
                    
                    # Richiedi all'utente il codice di autorizzazione
                ...
                if client_type == 'web':
                    ...
                    # Per le applicazioni desktop, usa il metodo integrato
